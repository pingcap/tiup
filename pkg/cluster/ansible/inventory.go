...
func ParseAndImportInventory(dir, ansCfgFile string, clsMeta *spec.ClusterMeta, inv *aini.InventoryData, sshTimeout int64, executorType string) error {
	...
	for i := 0; i < len(clsMeta.Topology.TiDBServers); i++ {
		...
		ins, err := parseDirs(clsMeta.User, s, sshTimeout, executorType)
		if err != nil {
			return err
		}
