...
function cmd_subtest() {
    ...
    for i in {1..5}; do "${ssh_cmd[@]}" "${node}${i}" "grep -q tidb /etc/passwd && (killall -u tidb; userdel -f -r tidb) || true"; done
    ...
    tiup-cluster "${client[@]}" exec $name -N ${node}1 --command "grep tidb.rules.yml /home/tidb/deploy/prometheus-9090/conf/prometheus.yml"
    ! tiup-cluster "${client[@]}" exec $name -N ${node}1 --command "grep node.rules.yml /home/tidb/deploy/prometheus-9090/conf/prometheus.yml"
    ...
    tiup-cluster "${client[@]}" exec $name -N ${node}1 --command "grep /home/tidb/deploy/tikv-20160/data /home/tidb/deploy/tikv-20160/scripts/run_tikv.sh"
    tiup-cluster "${client[@]}" exec $name -N ${node}1 --command "grep advertise-status-addr /home/tidb/deploy/tikv-20160/scripts/run_tikv.sh"
    tiup-cluster "${client[@]}" exec $name -N ${node}3 --command "grep /home/tidb/my_kv_data /home/tidb/deploy/tikv-20160/scripts/run_tikv.sh"

    # test checkpoint
    tiup-cluster "${client[@]}" exec $name -N ${node}1 --command "touch /tmp/checkpoint"
    tiup-cluster "${client[@]}" exec $name -N ${node}1 --command "ls /tmp/checkpoint"
    tiup-cluster "${client[@]}" exec $name -N ${node}1 --command "rm -f /tmp/checkpoint"
    ...
    tiup-cluster "${client[@]}" --yes clean $name --data --all --ignore-node ${node}1:9090
    ...
    tiup-cluster "${client[@]}" exec $name -N ${node}1 --command "ls /home/tidb/deploy/prometheus-9090/log/prometheus.log"
    ! tiup-cluster "${client[@]}" exec $name -N ${node}1 --command "ls /home/tidb/deploy/tikv-20160/log/tikv.log"
    ...
    ! "${ssh_cmd[@]}" -i "/tmp/$name.id_rsa" tidb@${node}1 "ls"
